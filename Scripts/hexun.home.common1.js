...
hexun.home.navBar = (function () {
    ...
    function bindEvent() {
        var $Search = jQuery("#" + defaults.searchID),//搜索DOM
...
hexun.home.banner = (function () {
    ...
    var initialize = function () {
        var oShow = jQuery("#" + defaults.showID),//展示DOM
            oParel = jQuery("#" + defaults.parelID),//面板DOM
            oClose = jQuery("#" + defaults.closeID);//关闭DOM
        //图片悬浮
...
hexun.home.bannerPic = (function () {
    ...
    function bindEvent() {
        var oBtn = jQuery('#' + defaults.btnID); // 导航条 图片 按钮
        // 开启 个性化设置
        oBtn.click(function () {
            loadData();
            return false;
        ...
        Jser.ajax({
            ...
            success: function (data) {
                if (data.status == 1) {
                    ...
                    cacheSrc = jQuery("#" + defaults.btnID).prev().attr("src");
                    ...
                    hexun.home.confirm().init({
                        ...
                        cancel: function () {
                            jQuery("#" + defaults.btnID).prev().attr("src", cacheSrc);
                        },
                        close: function () {
                            jQuery("#" + defaults.btnID).prev().attr("src", cacheSrc);
                        },
                        callback: function () {
                            ...
                            //绑定事件
                            jQuery("#hxjy_banner_list").find("li").live("click", function () {
                                jQuery("#hxjy_banner_list").find("li").each(function () {
                                    this.className = "";
                                });
                                this.className = "cover_select";
                                var src = jQuery(this.getElementsByTagName('img')[0]).attr("src").split(".jpg")[0] + "big.jpg";
                                jQuery("#" + defaults.btnID).prev().attr("src", src);
                                guid = src;
                            });
                            // 推荐封面
                            jQuery(Me.oAlert).find('ul.pop_window_sec a:last').click(function () {
                                Me.closeAlert();
                                var ret = [];
                                ret.push('<ul class="pop_window_sec">');
                                ...
                                hexun.home.confirm().init({
                                    ...
                                    callback: function () {
                                        var Me02 = this;
                                        jQuery(Me02.oAlert).find('ul.pop_window_sec a:first').click(function () {
                                            Me02.closeAlert();
                                            loadData();
                                            return false;
                                        });
...
hexun.home.page = (function () {
    var defaults = {
        ...
        callback: jQuery.noop //ajax状态下的 回调函数
    };
    var initialize = function () {
        var oPage = jQuery("#" + defaults.panelID),//分页
...
Class('scrollBars', null, {
    ...
    scrollBars: function (options) {
        var options = jQuery.extend(this.options, options || {});
        options.count = jQuery.makeArray(options.count);
        options.url = jQuery.makeArray(options.url);
    ...
    options: {                           // 选择器
        ...
        doDataHtml: jQuery.noop              // 将数据拼接为HTML
    },
    //初始化
    init: function () {
    ...
    bindEvent: function () {
        ...
        //绑定滚动事件
        jQuery(window).scroll(this.doScroll);
        // 检测是否需要手动滚动
        if (options.isManual) {
            var oManual = jQuery('#' + options.manualID);
    ...
    doScroll: function () {
        ...
        if (options.count[options.index] < options.max) {
            ...
            // 超过默认滚动次数
            jQuery(window).unbind('scroll', Me.doScroll);    // 解除绑定滚动事件
            // 检测是否需要手动滚动
            if (options.isManual) {
                var oManual = jQuery('#' + options.manualID);
                // 检测是否需要继续分页
    ...
    loadData: function () {
        var Me = this, options = Me.options, count = options.count[options.index], max = options.max,
            oManual = jQuery('#' + options.manualID), pageIndex, url;
        if (count < max) { // 加载数据过程中 解除绑定滚动事件
            jQuery(window).unbind('scroll', Me.doScroll);
        }
        ...
        Jser.ajax({
            ...
            success: function (data) {
                ...
                if (count < max && Me.isScroll) {  // 加载数据结束,重新绑定事件
                    jQuery(window).scroll(Me.doScroll);
                }
                Me.updateScene();           // 更新场景
            }
        });
...
Class('Alert', null, {
    ...
    creatAlert: function () {
        ...
        //检测背景层是否存在
        var oBg;
        oBg = jQuery("body").children(".layer");
        if (!oBg.length) {
            // 创建背景层
...
Class('Page', null, {
    ...
    init: function () {
        ...
        jQuery("#" + this.panelID).html(aHTML.join(""));
        this.bindEvent();
    },
    bindEvent: function () {
        var oPanel = jQuery("#" + this.panelID),
...
hexun.home.login = (function(){
    ...
    var initialize = function(){
        var $eleBox = jQuery('#' + defaults.logBoxId),
        ...
        $eleInputs.eq(0).focus(function(){
            jQuery(this.parentNode.parentNode).addClass(f).removeClass(e);
            jQuery(this).prev().html('');
            //jQuery(this.parentNode).next().hide();
        });
        $eleInputs.eq(0).blur(function(){
            var can = comVad.test(this.value),
                p =   jQuery(this.parentNode.parentNode);
            if(can || this.value == ''){
                p.addClass(e);
                jQuery(this).val('');
                jQuery(this).prev().html('用户名错误');
            }
            else{
                p.removeClass(f)
            }
            jQuery(this.parentNode).next().show();
        });
        // 密码事件
